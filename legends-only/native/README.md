...
- **Payments on iOS link out.** Vote packs, Fan Club, and memberships are digital goods, and Apple takes 30% of anything bought inside an iOS app. The app detects the iOS wrapper (`Capacitor.getPlatform() === 'ios'`) and opens every purchase in Safari at `/?buy=votes&pack=v20`, `/?buy=club`, or `/?buy=tier&tier=hof`; the web app signs the person in and finishes the purchase through Stripe, all revenue yours. Install `@capacitor/browser` so the handoff opens Safari rather than an in-app view: `npm install @capacitor/browser && npx cap sync`. Web and Android buy in-app. Tips already run on the public Legend page in the browser.
