...
| Stripe account | Separate | Separate |
| Brand system | Separate | Separate |
| Regulatory regime | Adult game | COPPA / GDPR-K |

Shared: **the SML house mark only.** Nothing else crosses between them — no code,
no accounts, no currency, no data, no SSO, no in-app cross-promotion to minors.
