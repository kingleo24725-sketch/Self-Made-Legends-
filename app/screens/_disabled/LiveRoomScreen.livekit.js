 ...
 *
 * ARCHIVED — NOT IN THE BUNDLE. Nothing imports this file, so Metro never
 * resolves it and the LiveKit packages it needs are not installed.
 *
 * Glam Rooms are switched off in v1 (`app/utils/config.js` -> FEATURES.rooms),
 * and shipping the LiveKit SDK for a switched-off feature meant carrying a
 * native module whose Expo config plugin was missing from app.json — a build
 * risk and an APK-size cost for code nobody could reach.
 *
 * TO RESTORE, all four steps, none optional:
 *   1. npm i @livekit/react-native livekit-client @livekit/react-native-expo-plugin
 *   2. add "@livekit/react-native-expo-plugin" to app.json -> expo.plugins
 *   3. git mv this file back to screens/LiveRoomScreen.js
 *   4. set extra.features.rooms = true and make a NEW build — a native module
 *      cannot be turned on by a flag alone.
 *
